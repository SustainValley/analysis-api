...
def generate_promotion_message(status: str, main_meeting_type: str, dayOfWeek: str, timeSlot: str) -> str:
    ...
    3. 상권 상태가 "활성화"일 때는 강한 프로모션보다는 가벼운 혜택이나 고객 만족도를 높이는 아이디어 위주로 제안.
    4. 상권 상태가 "비활성화"일 때는 집중적으로 고객을 유입할 수 있는 실질적인 프로모션을 제안.
    5. 어투는 자연스럽게, 간단히 1문장 정도로, 마지막에 “~하는 건 어떨까요?” 식으로 권고형/제안형 톤으로 마무리.
